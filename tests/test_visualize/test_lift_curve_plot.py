...
class TestLiftCurvePlot:
    ...
    @pytest.mark.parametrize("line_no, value", [
        (0, 19.5),
        (1, 19.5),
        (2, 19.5),
        (3, 1)
    ])
    def test_lift_curve_to_have_the_correct_xdata(self,
                                                  ax: Axes,
                                                  line_no: int,
                                                  value: float):
    ...
    def test_lift_curve_to_have_the_correct_ydata(self,
                                                  ax: Axes,
                                                  line_no: int,
                                                  value: float):
    ...
    def test_legend_uses_class_numbers_when_no_labels_are_passed(self,
                                                                 ax: Axes):
        """Expect the legend to default to numbers if no labels are passed"""
        expected = {'Class 0 $Lift = 3.17$ ',
                    'Class 1 $Lift = 2.70$ ',
                    'Class 2 $Lift = 2.70$ ',
                    'Baseline'}
    ...
    def test_legend_is_labelled_correctly_when_labels_are_passed(self,
                                                                 ax_labels: Axes):
        """Expect the legend to include labels"""
        expected = {'Class Setosa $Lift = 3.17$ ',
    ...
    def test_raises_if_wrong_number_of_labels_is_passed(self, iris_result: Result):
        ...
        with pytest.raises(VizError, match="Number of labels must equal number of classes"):
